...
import type { NewUserType } from "../types/authentication.types";
...
*/
		...
		const checkUser = db.prepare(`SELECT id, name, email, username FROM users WHERE id = ?`).get(req.session.user.id) as NewUserType;
